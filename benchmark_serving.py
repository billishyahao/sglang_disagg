...
def sample_random_requests(
    ...
    # Create a local RNG for retry-loop padding so we don't contaminate
    # the global random state (keeps serial and parallel paths deterministic).
    local_rng = np.random.RandomState(
        np.random.get_state()[1][:4].tolist()
    )
    ...
    if use_parallel:
        ...
        if tokenizer_id is None and num_workers > 1:
            print("Warning: tokenizer_id not provided, falling back to serial...")
        input_requests = []
        mismatches = []
